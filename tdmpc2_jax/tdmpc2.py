...
class TDMPC2(struct.PyTreeNode):
  ...
  def act(self,
    ...
    if self.mpc:
      ...
      action = self.model.sample_actions(
          z, carry, self.model.policy_model.params, key=action_key
      )[0]
      plan = None
      updated_carry = self.model.next(
            z, action, self.model.dynamics_model.params, carry)[1]
  ...
  #@jax.jit
  @partial(jax.jit, static_argnames=('train'))#in_axes=(None, 0, 0, None, 0, 0), out_axes=0)
  def plan(self,
           z: jax.Array,
    ...
    if self.policy_prior_samples > 0:
        ...
        carry_t = carry[..., None, :].repeat(self.policy_prior_samples, axis=-2)  # ADD THIS
        
        for t in range(self.horizon):
            policy_actions = policy_actions.at[..., t, :].set(
                self.model.sample_actions(
                    z_t, carry_t, self.model.policy_model.params, key=prior_noise_keys[t]  # ADD carry_t
                )[0]
            )
            if t < self.horizon-1:
                z_t, carry_t = self.model.next(  # UPDATE TO UNPACK TUPLE
                    z_t,
                    policy_actions[..., t, :],
    ...
    carries_t = carry[..., None, :].repeat(self.population_size, axis=-2)
    ...
    for i in range(self.mppi_iterations):
      ...
      # Compute elites
      values = self.estimate_value(z_t, actions, carries_t, key=value_keys[i])
  ...
  def update(self,
    ...
    def world_model_loss_fn(encoder_params: flax.core.FrozenDict,
      ...
      current_carry = hidden_states[0]
      initial_carry = jnp.zeros_like(current_carry)
      carries_rollout = jnp.zeros((self.horizon+1, self.batch_size, current_carry.shape[-1]))
      carries_rollout = carries_rollout.at[0].set(current_carry)
      ###########################################################
      ...
      next_action = self.model.sample_actions(
          next_z, carries_rollout[1:], self.model.policy_model.params, key=next_action_key
      )[0]
      Qs, _ = self.model.Q(
          next_z, next_action, carries_rollout[1:], self.model.target_value_model.params, key=Q_key
      )
    ...
    def policy_loss_fn(actor_params: flax.core.FrozenDict,):
      ...
      actions, _, _, log_probs = self.model.sample_actions(
          zs, carries, actor_params, key=action_key
      )
  ...
  def estimate_value(self,
                     z: jax.Array,
                     actions: jax.Array,
                     carry: jax.Array, key: PRNGKeyArray
                     ) -> jax.Array:
